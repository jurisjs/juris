...
class ComponentManager {
    ...
    #createLifecycleComponent(result, name, props, compStates) {
        ...
        const updateRender = () => {
            const { result: renderResult, deps } = this.juris.stateManager.track(() => {
                return instance.render ? instance.render(container) : result;
            });
        ...
        try {
            ...
            if (renderResult?.then) {
                ...
                promisify(renderResult).then(resolvedResult => {
                    this.#updateContainerContent(resolvedResult, container, name);
                }).catch(error => {
            ...
            this.#updateContainerContent(renderResult, container, name);
        } catch (error) {
    ...
    #updateContainerContent(renderResult, container, name) {
        const renderedContent = this.juris.domRenderer.render(renderResult);
        
        if (container.parentNode) {
            // Get instance data for API transfer
            ...
            if (renderedContent?.constructor === HTMLDivElement) {
                ...
                newContainer = document.createElement('div');
                newContainer.setAttribute('data-juris-reactive-render', name);
                if (renderedContent) newContainer.appendChild(renderedContent);
            }
            
            // Transfer API
            if (instance?.api) {
                newContainer.api = instance.api;
                this.instances.set(newContainer, instance);
                this.namedComponents.set(name, { element: newContainer, instance });
            }
            ...
            // No parent - just update content
            const children = Array.from(container.children);
            children.forEach(child => this.cleanup(child));
            container.innerHTML = '';
            if (renderedContent) container.appendChild(renderedContent);
        }
    }
...
class DOMRenderer {
    constructor(juris) {
        ...
        this.elementTypeCache = new Map(); // Cache for element type detection
    ...
    // Centralized async handler - the single function for all promise operations
    ...
    #createElementByType(tagName) {
        ...
        if (isSVG === undefined) {
            ...
            try {
                ...
                // SVG elements should have specific constructors, not generic SVGElement
                // Also exclude common HTML elements that exist in both namespaces
    ...
    _handleEvent(element, eventName, handler, eventListeners) {
        ...
        // Handle special cases, otherwise strip 'on' prefix
