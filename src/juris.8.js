...
class DOMRenderer {
    constructor(juris) {
        ...
        // Key diffing structures
        this.keyedNodes = new Map(); // element -> Map of key -> node
        this.nodeKeys = new Map();   // node -> key (reverse lookup)
    ...
    #diffChildren(parent, oldChildren, newChildren) {
        ...
        // Build new key map and collect operations
        const operations = [];
        ...
        for (let i = 0; i < newChildren.length; i++) {
            ...
            if (usedKeys.has(key)) {
                // Duplicate key - use indexed fallback
                const fallbackKey = `__dup_${key}_${i}`;
    ...
    #shouldUpdateNode(node, vnode) {
        // Simple heuristic - update if it's a text node with different content
        if (node.nodeType === Node.TEXT_NODE) {
            return node.textContent !== String(vnode);
        }
        
        // For element nodes, we'll always update to be safe since props might have changed
        // This could be optimized further by comparing the actual VDOM structure
    ...
    #executeChildOperations(parent, operations, newChildren) {
        ...
        // 4. Insert and position all nodes correctly
    ...
    #reorderNodes(parent, targetOrder) {
        ...
        targetOrder.forEach(targetNode => {
            if (!targetNode) return;
            
            const currentChild = parent.childNodes[currentIndex];
            
            if (currentChild !== targetNode) {
                if (targetNode.parentNode !== parent) {
                    // Insert new node
                    parent.insertBefore(targetNode, currentChild);
                } else {
                    // Move existing node
    ...
    #handleAsyncProp(element, propName, propValue) {
        ...
        return this.#handleAsync(propValue, {
            onStart: () => {
                this.#setPlaceholder(element, propName);
            },
            onResolved: (resolved) => {
    ...
    // Enhanced children handling with key-based diffing
    ...
    #updateChildren(element, children, componentName = null) {
        ...
        if (Array.isArray(children)) {
            // Check if we need key-based diffing
            const hasKeys = children.some(child => this.#extractKey(child) !== null);
            const hasReactiveFunctions = children.some(child => typeof child === 'function');
            
            if (hasKeys && !hasReactiveFunctions) {
                // Use key-based diffing for static keyed children
                const currentChildren = Array.from(element.childNodes)
                    .map((node, index) => {
                        if (node.nodeType === Node.TEXT_NODE) return node.textContent;
                        return node._jurisVNode || { __placeholder: true, index };
                    });
                    
                children.forEach(child => {
                    const renderedChild = this.#createChild(child, componentName);
                    if (renderedChild) {
                        renderedChild._jurisVNode = child;
                    }
                });
                
                this.#diffChildren(element, currentChildren, children);
                return;
            }
    ...
    #createArrayFragment(vnode, componentName) {
        ...
        if (hasReactiveFunctions || hasKeys) {
            ...
            if (hasKeys && !hasReactiveFunctions) {
                // Static keyed fragment - render normally but track keys
