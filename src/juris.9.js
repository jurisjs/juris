...
class DOMRenderer {
    ...
    #reorderNodes(parent, targetOrder) {
        ...
        currentNodes.forEach((node, index) => nodePositions.set(node, index));
        let lastNode = null;        
    ...
    #updateChildren(element, children, componentName = null) {
        ...
        if (Array.isArray(children)) {
            ...
            if (hasKeys) {
                // Use keyed diffing regardless of reactive functions
                this.#diffKeyedChildren(element, children, componentName);
    ...
    #diffKeyedChildren(parent, children, componentName) {
        const processedChildren = children.map((child, index) => {
            if (typeof child === 'function') {
                // Wrap reactive children in a stable keyed container
                const key = `__reactive_${index}`;
                return { div: { key, children: child } };
            }
            return child;
        });
        
        this.#diffChildren(parent, Array.from(parent.childNodes), processedChildren);
    }
