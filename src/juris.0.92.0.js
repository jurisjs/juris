...
class ComponentManager {
    ...
    #processComponentResult(result, name, props, compStates) {
        ...
        if (result && typeof result === 'object') {
            ...
            if (keys.length === 1 && typeof keys[0] === 'string' && keys[0].length > 0) {
                const element = this.juris.domRenderer.render(result, name);
                if (element && compStates.size > 0) {
                    this.compStates.set(element, compStates);
                }
                if (result.api && typeof result.api === 'object' && element) {
                    element.api = result.api;
                }
                if (element && element.setAttribute) {
                    element.setAttribute('data-juris-component', name);
                    element._jurisComponent = name;
                }
                return element;
            }
        }
    ...
    #createLifecycleComponent(result, name, props, compStates) {
        ...
        container.setAttribute('data-juris-component', name);
        const updateRender = () => {
            const { result: renderResult, deps } = this.juris.stateManager.track(() => {
                return instance.render ? instance.render() : result;
            });
            const renderedContent = this.juris.domRenderer.render(renderResult, name, props);
            if (container.parentNode) {
                if (container._reactiveSubscriptions) {
                    container._reactiveSubscriptions.forEach(unsub => unsub());
                }
                let newContainer;
                if (renderedContent.constructor === HTMLDivElement) {
                    newContainer = renderedContent;
                } else {
                    newContainer = document.createElement('div');
                    newContainer.setAttribute('data-juris-component', name);
                    newContainer.appendChild(renderedContent);
                }
            ...
            deps.forEach(path => {
                const unsub = this.juris.stateManager.subscribeInternal(path, updateRender);
                // Store subscription for cleanup
                if (!container._reactiveSubscriptions) {
                    container._reactiveSubscriptions = [];
                }
                container._reactiveSubscriptions.push(unsub);
            });
        };
        ...
        if (container) {
            this.#setupComponentInstance(container, instance, compStates, name);
            if (instance.api && typeof instance.api === 'object') {
                container.api = instance.api;
            }
        }
    ...
    #executeReactiveRender(renderFn, container, name) {
        try {
            const renderResult = renderFn(container);
...
class DOMRenderer {
  constructor(juris) {
      ...
      // VanJS-inspired property handling
      this.propSetterCache = {};
      
      // Framework-specific attributes to skip
      ...
      // Auto-cleanup for disconnected elements
      this._setupAutoCleanup();
  }

  // VanJS-style property descriptor lookup with caching
  _getPropDescriptor(proto, propName) {
      return proto ? 
          Object.getOwnPropertyDescriptor(proto, propName) ?? 
          this._getPropDescriptor(Object.getPrototypeOf(proto), propName) :
          undefined;
  }

  _getCachedPropSetter(element, attrName) {
      const tagName = element.tagName.toLowerCase();
      const cacheKey = `${tagName},${attrName}`;
      
      let propSetter = this.propSetterCache[cacheKey];
      if (propSetter === undefined) {
          const descriptor = this._getPropDescriptor(Object.getPrototypeOf(element), attrName);
          propSetter = this.propSetterCache[cacheKey] = descriptor?.set ?? 0;
      }
      
      return propSetter;
  }

  // Enhanced attribute setting with proper validation
  _setStaticAttribute(element, attr, value) {
      if (this.SKIP_ATTRS.has(attr)) return;
      
      // Handle className -> class mapping
      const actualAttr = attr === 'className' ? 'class' : attr;
      
      // Boolean property handling
      if (attr in element && typeof element[attr] === 'boolean') {
          const boolValue = value && value !== 'false';
          element[attr] = boolValue;
          if (boolValue) {
              element.setAttribute(actualAttr, '');
          } else {
              element.removeAttribute(actualAttr);
          }
          return;
      }
      
      // SVG namespace - always use setAttribute
      if (element.namespaceURI === 'http://www.w3.org/2000/svg') {
          element.setAttribute(actualAttr, value);
          return;
      }
      
      // Special attribute patterns (optimized character code checking)
      ...
      // Use VanJS approach: check for property setter
      const propSetter = this._getCachedPropSetter(element, attr);
      
      if (propSetter) {
          // Has valid setter - use property assignment
          element[attr] = value;
      } else {
          // No setter or not a property - use attribute
          element.setAttribute(actualAttr, value);
      }
  }

  // Centralized async handler
  _handleAsync(value, handlers = {}) {
      const {
          onStart = () => {},
          onResolved = () => {},
          onError = (error) => {
  ...
  // Enhanced element creation with cached type detection
  _createElementByType(tagName) {
  ...
  // Reactive handler creation
  _createReactiveHandler(element, getValue, updateDom, options = {}) {
  ...
  _renderToDOM(vnode, componentName = null) {
      ...
      if (Array.isArray(vnode)) {
          return this._createArrayFragment(vnode, componentName);
      }
      ...
      if (this.componentStack.includes(tagName)) {
          return this._createErrorElement('recursion', [...this.componentStack, tagName].join(' → '));
      }       
      if (this.juris.componentManager.components.has(tagName)) {
          return this._renderComponent(tagName, props);
      }       
      if (/^[A-Z]/.test(tagName)) {
          return this._createErrorElement('component', `Component "${tagName}" not registered`);
      }
      if (typeof tagName !== 'string' || tagName.length === 0) return null;

      // Handle custom CSS if available
      ...
      return this._createElement(tagName, modifiedProps, componentName);
  }

  _createElement(tagName, props, componentName = null) {
      const element = this._createElementByType(tagName);
  ...
  // Property application with enhanced handling
  applyProp(element, propName, propValue, componentName = null) {
      ...
      } else if (this._isPromiseLike(propValue)) {
          this._handleAsyncProp(element, propName, propValue);
      } else {
  ...
  _handleText(element, text, subscriptions) {
      if (typeof text === 'function') {
          const updateText = this._createReactiveHandler(
              element,
              () => text(element),
  ...
  _handleStyle(element, style, subscriptions) {
      if (typeof style === 'function') {
          const updateStyle = this._createReactiveHandler(
              element,
              () => {
          ...
          this._handleAsyncStyle(element, style);
      } else if (typeof style === 'object') {
          this._applyStyleObject(element, style, subscriptions);
      }
  }

  _handleAsyncStyle(element, style) {
      const config = this._getPlaceholderConfig(element);
      
      this._handleAsync(style, {
          onStart: () => {
              element.classList.add(config.className);
              if (config.style) {
                  this._applyStyleString(element, config.style);
              }
          },
          onResolved: (resolved) => {
              element.classList.remove(config.className);
              if (typeof resolved === 'object') {
                  if (config.style) {
                      this._removeStyleString(element, config.style);
                  }
          ...
          onError: (error) => {
              element.classList.add('juris-async-error');
              log.ee && console.error(log.e('Style loading failed:', error), 'application');
          }
      });
  }

  _applyStyleObject(element, style, subscriptions) {
      for (const prop in style) {
          if (style.hasOwnProperty(prop)) {
              ...
              if (typeof val === 'function') {
                  ...
                  this._setStyleProperty(element, prop, val);
              }
          }
      }
  }
  ...
  _handleReactiveStyleProperty(element, prop, valueFn, subscriptions) {
      const updateStyleProperty = this._createReactiveHandler(
          element,
          () => valueFn(element),
          (value) => this._setStyleProperty(element, prop, value),
          { trackChanges: true, name: `style property '${prop}'` }
      );
      this._createReactiveUpdate(element, updateStyleProperty, subscriptions);
  }

  _setStyleProperty(element, prop, value) {
      if (prop.startsWith('--')) {
  ...
  _applyStyleString(element, styleString) {
      const rules = styleString.split(';').filter(s => s.trim());
      rules.forEach(rule => {
          const [prop, value] = rule.split(':').map(s => s.trim());
          if (prop && value) {
              this._setStyleProperty(element, prop, value);
          }
      });
  }

  _removeStyleString(element, styleString) {
  ...
  _handleReactiveAttribute(element, attr, valueFn, subscriptions) {
      const updateAttribute = this._createReactiveHandler(
          element,
          () => valueFn(element),
  ...
  _handleEvent(element, eventName, handler, eventListeners) {
      ...
      if (eventName === 'onclick') {
          this._attachTouchSupport(element, handler, eventListeners);
      }
  }

  _attachTouchSupport(element, handler, eventListeners) {
  ...
  _handleAsyncProp(element, propName, propValue) {
      const config = this._getPlaceholderConfig(element);
      
      if (propName === 'innerHTML') {
          return this._handleAsync(propValue, {
              onStart: () => {
      ...
      return this._handleAsync(propValue, {
          onStart: () => {
              this._setPlaceholder(element, propName);
          },
          onResolved: (resolved) => {
              const config = this._getPlaceholderConfig(element);
              element.classList.remove(config.className);
              this._setStaticAttribute(element, propName, resolved);
          },
          onError: (error) => {
              this._setErrorState(element, propName, error.message);
          }
      });
  }
  ...
  _handleChildren(element, children, subscriptions, componentName = null) {
      if (typeof children === 'function') {
          ...
          this._handleAsyncChildren(element, children);
      } else {
          this._updateChildren(element, children, componentName);
      }
  }
              ...
              onResolved: (resolved) => {
                  if (result !== "ignore") {
                      if (typeof resolved === 'string' || typeof resolved === 'number') {
                          ...
                          this._updateChildren(element, resolved);
                      }
                  }
              },
              onError: (error) => {
  ...
  _handleAsyncChildren(element, children) {
      const config = this._getPlaceholderConfig(element);
      
      this._handleAsync(children, {
          onStart: () => {
              let placeholder;
              if (config.children) {
              ...
              this.placeholders.set(element, { type: 'children', placeholder });
          },
          onResolved: (resolved) => {
              const asyncData = this.placeholders.get(element);
              if (asyncData?.placeholder?.parentNode) {
                  element.removeChild(asyncData.placeholder);
              }
              this._updateChildren(element, resolved);
              this.placeholders.delete(element);
          },
  ...
  _updateChildren(element, children, componentName = null) {
      ...
      if (Array.isArray(children)) {
          ...
          if (hasReactiveFunctions) {
              this._renderReactiveChildren(element, children, componentName);
          } else {
              this._renderStaticChildren(element, children, componentName);
          }
      } else if (children) {
          element.textContent = '';
          if (typeof children === 'function') {
              this._setupSingleReactiveChild(element, children, componentName);
          } else {
              const childElement = this._createChild(children, componentName);
  ...
  _renderStaticChildren(element, children, componentName) {
      element.textContent = '';
      const fragment = document.createDocumentFragment();
      for (let i = 0; i < children.length; i++) {
          const child = children[i];
          if (child != null) {
              const childElement = this._createChild(child, componentName);
  ...
  _renderReactiveChildren(element, children, componentName) {
      element.textContent = '';
      ...
      for (let i = 0; i < children.length; i++) {
          ...
          if (typeof child === 'function') {
              const { node, cleanup } = this._createIndividualReactiveChild(child, i, componentName, element);
              if (node) {
  ...
  _setupSingleReactiveChild(element, childFn, componentName) {
      ...
      const update = () => {
          ...
          this._handleAsync(result, {
              ...
              onResolved: (resolved) => {
                  element.textContent = '';
                  const childElement = this._createChild(resolved, componentName, false);
                  if (childElement) element.appendChild(childElement);
              },
              onError: (error) => {
                  element.textContent = `Error: ${error.message}`;
  ...
  _createIndividualReactiveChild(childFn, index, componentName, parentElement) {
      ...
      const updateThisChild = () => {
          ...
          this._handleAsync(result, {
              ...
              onResolved: (resolved) => {
                  const newNode = this._createChild(resolved, componentName) || document.createTextNode('');
                  if (currentNode.parentNode) {
  ...
  _createChild(child, componentName) {
      ...
      if (Array.isArray(child)) {
          ...
          for (let i = 0; i < child.length; i++) {
              const subChild = this._createChild(child[i], componentName);
  ...
  _createArrayFragment(vnode, componentName) {
      const hasReactiveFunctions = vnode.some(item => typeof item === 'function');
      
      if (hasReactiveFunctions) {
          const fragment = document.createDocumentFragment();
          const subscriptions = [];
          this._handleReactiveFragmentChildren(fragment, vnode, subscriptions);
          if (subscriptions.length > 0) {
  ...
  _handleReactiveFragmentChildren(fragment, children, subscriptions) {
      for (let i = 0; i < children.length; i++) {
          const child = children[i];
          if (typeof child === 'function') {
              let currentNode = document.createTextNode('');
              ...
              const updateChild = () => {
                  ...
                  this._handleAsync(result, {
                      ...
                      onResolved: (resolved) => {
                          const newNode = this._createChild(resolved, null, false) || document.createTextNode('');
                          if (currentNode.parentNode) {
                              currentNode.parentNode.replaceChild(newNode, currentNode);
                          }
                          currentNode = newNode;
              ...
              const childElement = this._createChild(child, null, false);
              if (childElement) {
                  fragment.appendChild(childElement);
              }
          }
  ...
  _renderComponent(tagName, props) {
      ...
      const { result, deps } = this.juris.stateManager.track(() => 
          this.juris.componentManager.create(tagName, props), true);
      this.componentStack.pop();
      return result;
  }

  _createErrorElement(type, message) {
      const element = document.createElement('div');
      element.style.cssText = 'color: red; border: 1px solid red; padding: 8px; background: #ffe6e6; font-family: monospace;';
  ...
  // Test mode and object tree support
