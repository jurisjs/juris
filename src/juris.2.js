...
class ComponentManager {
    ...
    #createReactiveRenderComponent(result, name, props, compStates) {
        ...
        this.instances.set(virtualContainer, componentData);
        if (result.api) {
            this.namedComponents.set(name, { element: virtualContainer, instance: componentData });
        }
        const updateRender = () => this.#executeReactiveRender(result.render, virtualContainer, name);
        const subscriptions = [];
        this.juris.domRenderer._createReactiveUpdate(virtualContainer, updateRender, subscriptions);
        if (subscriptions.length > 0) {
            this.juris.domRenderer.subscriptions.set(virtualContainer, { 
                subscriptions, 
                eventListeners: [] 
            });
        }
        if (compStates?.size > 0) {
            this.compStates.set(virtualContainer, compStates);
        }
...
class Juris {
    ...
    getRegisteredWebComponents() {
        return this.webComponentFactory ? this.webComponentFactory.getRegisteredComponents() : [];
    }

    isWebComponentRegistered(tagName) {
        return this.webComponentFactory ? this.webComponentFactory.isRegistered(tagName) : false;
    }
