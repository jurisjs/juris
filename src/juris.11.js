...
let st=setTimeout,ct=clearTimeout;
let createLogger = () => {
    let s = [];
    let f = (m, c, cat) => {
        let msg = `${cat ? `[${cat}] ` : ''}${m}${c ? ` ${JS(c)}` : ''}`;
        let logObj = { formatted: msg, message: m, context: c, category: cat, timestamp: Date.now() };
        st(() => s.forEach(sub => sub(logObj)), 0);
        return logObj;
    };
    return {
...
let createPromisify = () => {
    ...
    let trackingPromisify = result => {
        let promise = typeof result?.then === FN ? result : Promise.resolve(result);
        if (isTracking && promise !== result) {
            activePromises.add(promise);
            promise.finally(() => {
                activePromises.delete(promise);
                st(checkAllComplete, 0);
            });
        }
        return promise;
    };
...
class StateManager {
    constructor(initialState = {}, middleware = []) {
        ...
        this.initialState = JSON.parse(JS(initialState));
    ...
    addPlugin(name, plugin) {
        ...
        if (plugin.initialize && typeof plugin.initialize === FN) {
    ...
    removePlugin(name) {
        ...
        if (plugin && plugin.destroy && typeof plugin.destroy === FN) {
            plugin.destroy();
        }
    ...
    reset() {
        ...
        this.state = JSON.parse(JS(this.initialState));
        this.pathCache.clear();
        
        // Add this:
        this.plugins.forEach(plugin => {
            if (plugin.reset && typeof plugin.reset === FN) {
                plugin.reset();
            }
        });
    }
    ...
    executeBatch(callback) {
        ...
        try {
            let result = callback();
            if (result && typeof result.then === FN) {
                return result
    ...
    #setStateFast(path, value) {
        ...
        for (let i = 0; i < lastIndex; i++) {
            let part = parts[i];
            if (current[part] == null || typeof current[part] !== OB) {
                current[part] = {};
            }
...
class ComponentManager {
    ...
    #procCompResult(result, name, props, states) {
        ...
        if (result && typeof result === OB) {
            if (this.#hasHooks(result)) {
                return this.#newHooksComp(result, name, props, states);
            }
            if (typeof result.render === FN && !this.#hasHooks(result)) {
            ...
            if (keys.length === 1 && typeof keys[0] === 'string' && keys[0].length > 0) {
                ...
                if (result.api && typeof result.api === OB && el) {
                    el.api = result.api;
                }
    ...
    #newHooksComp(result, name, props, states) {
        ...
        if (cont) {
            ...
            if (inst.api && typeof inst.api === OB) {
                cont.api = inst.api;
            }
        }
    ...
    #newRenderComp(result, name, props, states) {
        ...
        if (result.api && typeof result.api === OB) {
            cont.api = result.api;
        }
        if (result.api) {
    ...
    #setupComp(el, inst, states, name) {
        ...
        if (inst.hooks.onMount) {
            st(() => this.#runHook(inst.hooks.onMount, el, name, 'onMount'), 0);
        }
    }
    ...
    #runHook(hook, args, componentName, hookName) {
        try {
            let result = isA(args) ? hook(...args) : hook(args);
            if (result?.then) {
    ...
    #newKey(props) {
        return JS(props, (key, value) => value?.then ? '[Promise]' : value);
    }
...
class DOMRenderer {
    ...
    #handleAsync = (promise, handlers = {}, context = {}) => {
        // Only handle actual promises - caller should check #isPromiseLike first
    ...
    #updateExistingNode(node, vnode) {
        ...
        if (node.nodeType === Node.ELEMENT_NODE && typeof vnode === OB && !isA(vnode)) {
            let tagName = Object.keys(vnode)[0];
            let props = vnode[tagName] || {};
            let subscriptions = [];            
    ...
    _renderToDOM(vnode, componentName = null) {
        ...
        if (!vnode || typeof vnode !== OB) return null;        
        if (isA(vnode)) {
    ...
    #createArrayFragment(vnode, componentName) {
        let hasReactiveFunctions = vnode.some(item => typeof item === FN);
        ...
        if (typeof style === FN) {
            let updateStyle = this.#createReactiveHandler(
                ...
                () => {
                    ...
                    if (this.cssExtractor?.postProcessReactiveResult && typeof result === OB) {
            ...
            this.#handleAsync(style, {
                onResolved: (resolved) => {
                    if (typeof resolved === OB) {
                        Object.assign(elm.style, resolved);
                    }
                }
            }, asyncContext);
        } else if (typeof style === OB) {
    ...
    _setStaticAttribute(elm, attr, value) {
        ...
        if (attr in elm && typeof elm[attr] !== FN) {
            elm[attr] = value;
        } else {
            elm.setAttribute(attr, value);
        }
    ...
    clearCSSCache() {
        if (this.cssExtractor && typeof this.cssExtractor.clearCache === FN) {
...
class Juris {
    ...
    #detectGlobalAndWarn() {
        if (!Juris._done) { (requestIdleCallback || st)(() => { if (Juris.#inGlobal) return; Juris.#inGlobal = true; for (let key in globalThis) { if (globalThis[key] instanceof Juris) { log.ew && console.warn(`JURIS GLOBAL: '${key}'`); } } }); }
    }
    ...
    render(container = '#app') {
        ...
        try {
            if (isA(this.layout)) {
                let hasReactiveFunctions = this.layout.some(item => typeof item === FN);
