...
const jurisMinifiedSize = '41 kB, 11.5 kB gzipped';
...
class DOMRenderer {
    ...
    #createElement(tagName, props, componentName = null) {
        ...
        if (componentName) {
            element.setAttribute('data-juris-component', componentName);
            element._jurisComponent = componentName;
        }
   ...
   #handleReactiveChildren(element, childrenFn, subscriptions) {
    ...
    #updateChildren(element, children, componentName = null) {
        ...
        if (Array.isArray(children)) {
            ...
            if (this.#shouldUseDiffing(existingNodes, children)) {
                this.#applyKeyedDiff(element, children, componentName);
                return;
            }            
    ...
    #createArrayFragment(vnode, componentName) {
        ...
        for (let i = 0; i < vnode.length; i++) {
            ...
            if (childElement) {
                // Set key if present
                const key = this.#extractKey(child);
                if (key !== null) {
                    if (childElement.setAttribute) {
                        childElement.setAttribute('data-key', key);
                    }
                    childElement._jurisKey = key;
                }
    ...
    #renderComponent(tagName, props) {
        ...
        if (result && result.setAttribute) {
            result.setAttribute('data-juris-component', tagName);
            result._jurisComponent = tagName;
        }
        return result;
    }
    ...
    #createChild(child, componentName) {
        ...
        if (typeof child === 'object' && child !== null) {
            const element = this.render(child, componentName);
    ...
    #shouldUseDiffing(existingNodes, newChildren) {
        if (existingNodes.length === 0 || newChildren.length === 0) {
            return false;
        }
        
        // Check size difference (keep your original logic)
        const sizeDiff = Math.abs(existingNodes.length - newChildren.length);
        if (sizeDiff > Math.max(existingNodes.length, newChildren.length) * 0.5) {
            return false;
        }
        
        // Extract keys (keep your original logic but avoid redundant extraction)
        const existingKeys = existingNodes.map(node => 
            node._jurisKey || node.getAttribute?.('data-key') || null
        );
        const newKeys = newChildren.map(child => this.#extractKey(child));
        
        // Check if we have enough keys (keep your original logic)
        const existingHasKeys = existingKeys.some(k => k !== null);
        const newHasKeys = newKeys.some(k => k !== null);
        
        if (!existingHasKeys && !newHasKeys) {
            return false;
        }
        
        // Simplified overlap check - just see if any keys match
        // This replaces your expensive Set intersection logic
        const existingKeySet = new Set(existingKeys.filter(k => k !== null));
    ...
    #applyKeyedDiff(element, newChildren, componentName) {
        const existingNodes = Array.from(element.childNodes);
        const existingKeys = existingNodes.map(node => 
            node._jurisKey || node.getAttribute?.('data-key') || null
        );
        const newKeys = newChildren.map(child => this.#extractKey(child));
        const keyToNodeMap = new Map();
        ...
        for (let i = 0; i < newChildren.length; i++) {
            ...
            if (newKey !== null && keyToNodeMap.has(newKey)) {
                const existingNode = keyToNodeMap.get(newKey);
                nodesToRemove.delete(existingNode);
                const targetNode = element.childNodes[currentIndex];
                if (existingNode !== targetNode) {
                    element.insertBefore(existingNode, targetNode);
                }
                this.#updateNodeContent(existingNode, newChild, componentName);
            } else {
                const newNode = this.#createChild(newChild, componentName);
                if (newNode) {
                    if (newKey !== null) {
                        if (newNode.setAttribute) {
                            newNode.setAttribute('data-key', newKey);
                        }
                        newNode._jurisKey = newKey;
                    }                    
                    const targetNode = element.childNodes[currentIndex];
                    if (targetNode) {
    ...
    #updateNodeContent(existingNode, newChild, componentName) {
        if (typeof newChild === 'string' || typeof newChild === 'number') {
            const newContent = String(newChild);
            if (existingNode.textContent !== newContent) {
                existingNode.textContent = newContent;
            }
        } else if (newChild && typeof newChild === 'object' && !Array.isArray(newChild)) {
            const tagName = Object.keys(newChild)[0];
            const props = newChild[tagName] || {};
            for (const key in props) {
                if (key !== 'key' && key !== 'children') {
                    this.applyProp(existingNode, key, props[key], componentName);
                }
            }
            if (props.children) {
                this._handleChildren(existingNode, props.children, [], componentName);
            }
        }
    }
    #isPromiseLike(value) {
